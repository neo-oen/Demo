...
//        _syncHttpProtoc = [[syncHttpProtoc alloc] initWithURLString:@"http://219.143.33.51:8001/uabconfig.uab"];
	...
	//ZBLog(@"SyncEngineImpl: sync thread created.");
	[syncThread start];
	return self;
}
...
-(void)syncTimeout {
	ZBLog(@"SyncEngineImpl: syncTimeout called.");
//    [[MemAddressBook getInstance] test];
}

// sync engine thread routine method.
-(void)syncEngineStart {
	//ZBLog(@"SyncEngineImpl: sync engine routine start.");
...
//    [self syncTimeout];
	while (TRUE) {
		...
		//ZBLog(@"SyncEngineImpl: next step within runloop.");
	}
exit:
	[pool release];
	//ZBLog(@"SyncEngineImpl: sync engine routine quit.");
}


- (void)reAuth{
    [_syncHttpProtoc reAuth];
}

- (void)notifyAction:(SEL)aSelector {
	[self performSelector:aSelector onThread:syncThread withObject:nil waitUntilDone:NO];
	//ZBLog(@"SyncEngineImpl:  notifyAction.");
}

#pragma mark ///// startSyncAction (云端查询) //////

-(void)userCloudSummary
{
    if (_syncHttpProtoc != nil) {
        [_syncHttpProtoc startTaskRequest:TASK_USERCLOUDSUMMARY];
    }
}
...
{
	//ZBLog(@"SyncEngineImpl: startSyncAction.");
    if (_syncHttpProtoc != nil) {
