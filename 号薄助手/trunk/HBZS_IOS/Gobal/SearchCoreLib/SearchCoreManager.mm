...
- (void)AddContact:(NSNumber*)localID name:(NSString*)name phone:(NSArray*)phoneArray {
    ...
    u2char nameBuf[name.length + 1];   //typedef unsigned short u2char;
    
    [self string_u2char:name u2char:nameBuf];  //name 存储在nameBuf
    
    u2char phoneBuf[phoneStr.length + 1];
    [self string_u2char:phoneStr u2char:phoneBuf];  //phone 存储在phoneBuf
    
    Tree_AddData(&iSearchTree,[localID intValue],nameBuf,phoneBuf);   //建立搜索树
...
////added by kevin
//- (void)GetPinYin:(NSNumber *)localID pinyin:(NSMutableString *)pinyinDes{
//    [pinyinDes setString:@""];
//    
//    u2char pinyinBuf[256];
//    
//	BOOL result = Tree_GetPinYin(&iSearchTree,[localID intValue],pinyinBuf,nil);
//    
