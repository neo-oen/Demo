...
- (id)initWithURLString:(NSString *)strServerURL
{
    ...
    // 根据服务端要求，暂时客户端版本号用1120102100
    clientVersion = [[NSString alloc] initWithString:@"1120102100"];
	
    arrayTaskString = [[NSArray alloc] initWithObjects:@"unknown",@"ALL_UP",@"ALL_DOWN",@"SYNC_UP",@"SYNC_DOWN",@"SYNC",@"SLOW_SYNC",@"AUTHEN",nil];
...
- (void)syncStatusRefresh:(BOOL) waitDone
{
    ...
    if (sse.taskType >= 1 && sse.taskType <=6 && (sse.status == Sync_State_Success || sse.status == Sync_State_Faild) && [SettingInfo getIsLogged] == YES)
    {
        // 获取当前已经连续上传日志的次数
        ...
        vosLog(@"plain log report..%d", logUploadTimes);
        ...
        //vosLog(@"logFilePath...%@",logFilePath);
        ...
        if (error){
            vosLog(@"localizedFailureReason:%@", [error localizedFailureReason]);
        }
        ...
        [logString release];
        vosLog(@"plain log report finish.");
    }
...
- (void)changeProxyStatus
{
    ...
    [[ConfigMgr getInstance] setValue:[NSString stringWithFormat:@"%d", needProxy]
                            ...
                            forDomain:[NSString stringWithFormat:@"SyncServerInfo"]];
    vosLog(@"**********changeProxyStatus:%d**********", needProxy);
}
...
- (void)deviceSign
{
    ...
    NSString *cfuuid = [NSString stringWithFormat:@"%@",
...
- (void)startTaskRequest:(SyncTaskType)syncTaskType
{
	if (currentRequest){
		if ([currentRequest isExecuting]){
			vosLog(@"cannot start new task【%@】，old task in running【%@】", [arrayTaskString objectAtIndex:syncTaskType], [arrayTaskString objectAtIndex:currentRequest.syncTaskType]);
			return;
		}
	}
	...
	vosLog(@"start task:【%@】...", [arrayTaskString objectAtIndex:syncTaskType]);
	...
	vosLog(@"startTaskRequest:%@", [arrayTaskString objectAtIndex:syncTaskType]);
...
- (void)syncResponseHandler
{
	...
	while (TRUE){
        vosLog(@"syncHttpProtoc:");
		[[NSRunLoop currentRunLoop] runMode:NSDefaultRunLoopMode beforeDate:[NSDate distantFuture]];
		vosLog(@"syncHttpProtoc: next step within runloop.");
	}
exit:
	[pool release];
	vosLog(@"syncHttpProtoc: sync engine routine quit.");
}

- (void)syncHandlerTimeout
{
	vosLog(@"syncHttpProtoc: syncHandlerTimeout called.");
}
...
- (void)cancelCurrentTask
{
	if (currentRequest){
		if ([currentRequest isExecuting]){
			...
			vosLog(@"%@:user cancel task。", [arrayTaskString objectAtIndex:currentRequest.syncTaskType]);
		}
	}
	else{
		vosLog(@"canceling task : currentRequest unvalid");
	}
}
...
- (void)requestFinishedHandler:(ASIHTTPRequest *)request
{
    if ([request isCancelled] && ![request isFinished]){
        ...
        [self syncStatusRefresh:YES];
		vosLog(@"%@:uner cancel task。task failed。", [arrayTaskString objectAtIndex:request.syncTaskType]);
		return;
	}
...
- (void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    if ([request isCancelled]){
		sse.reason = GENERAL_ERR_USER_CANCEL;
		vosLog(@"%@:任务失败。原因:用户已取消任务。", [arrayTaskString objectAtIndex:request.syncTaskType]);
	}
	else{
		sse.reason = [error code];
        vosLog(@"%@:任务失败。reason=%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
    ...
    switch (request.protocStatus)
	{
        case PROTOC_STATUS_AUTHEN://认证失败
			vosLog(@"%@:认证失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
		case PROTOC_STATUS_DOWNLOAD_CONTACT://下载联系人失败
            vosLog(@"%@:下载联系人失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
        case PROTOC_STATUS_GET_CONTACTLIST_SUM://获取摘要失败
            vosLog(@"%@:获取摘要失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
			break;
		case PROTOC_STATUS_GET_VERSION://获取联系人版本号失败
			vosLog(@"%@:获取联系人版本号失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
			break;
        case PROTOC_STATUS_DOWNLOAD_PROTRAIT://下载联系人头像失败
            vosLog(@"%@:下载联系人头像失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
        case PROTOC_STATUS_UPLOAD_ALL://上传所有联系人失败
            vosLog(@"%@:上传所有联系人失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
        case PROTOC_STATUS_UPLOAD_PROTRAIT://上传联系人头像失败
            vosLog(@"%@:上传联系人头像失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
        case PROTOC_STATUS_UPLOAD_DIFFER_CONTACT://同步上传联系人失败
            vosLog(@"%@:同步上传联系人失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
        case PROTOC_STATUS_SYNC_PROTRAIT://头像摘要同步失败
            vosLog(@"%@:头像摘要同步失败. Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [error code]);
            break;
		default:
			vosLog(@"requestFailed req=%@", [request error]);
			break;
	}
}
...
- (void)procABFailed:(ASIHTTPRequest *)request ErrCode:(int32_t)errCode
{
    ...
    vosLog(@"%@:task failed。reason=%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
    ...
    switch (request.protocStatus)
	{
		case PROTOC_STATUS_DOWNLOAD_CONTACT://下载联系人失败
            ...
            vosLog(@"%@:下载联系人失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
        case PROTOC_STATUS_GET_CONTACTLIST_SUM://获取摘要失败
            vosLog(@"%@:获取摘要失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
			break;
		case PROTOC_STATUS_GET_VERSION://获取联系人版本号失败
			vosLog(@"%@:获取联系人版本号失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
			break;
        case PROTOC_STATUS_DOWNLOAD_PROTRAIT://下载联系人头像失败
            vosLog(@"%@:下载联系人头像失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
        case PROTOC_STATUS_UPLOAD_ALL://上传所有联系人失败
            [[MemAddressBook getInstance] rollback];
			vosLog(@"%@:上传所有联系人失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
        case PROTOC_STATUS_UPLOAD_PROTRAIT://上传联系人头像失败
            vosLog(@"%@:上传联系人头像失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
        case PROTOC_STATUS_UPLOAD_DIFFER_CONTACT://同步上传联系人失败
            [[MemAddressBook getInstance] rollback];
			vosLog(@"%@:同步上传联系人失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
        case PROTOC_STATUS_SYNC_PROTRAIT://头像摘要同步失败
            vosLog(@"%@:头像摘要同步失败(procABFailed). Error Code:%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
            break;
		default:
			vosLog(@"requestFailed req=%@", [request error]);
			break;
...
- (void)procPortraitFailed:(ASIHTTPRequest *)request ErrCode:(int32_t)errCode
{
    ...
    sse.reason = errCode;
	vosLog(@"%@:任务失败。reason=%d", [arrayTaskString objectAtIndex:request.syncTaskType], errCode);
...
- (void)saveConfigUrl{
    NSArray *configResponseArray = [NSArray arrayWithObjects:[configResponse authUrl],
                                    ...
                                    [configResponse getUserCloudSummaryUrl],nil];
    NSArray *keyArray = [NSArray arrayWithObjects:@"authUrl",@"uploadPortraitUrl",
                         @"downloadPortraitUrl",
                         @"uploadAllUrl",@"getContactListUrl",
                         @"downloadContactUrl",
                         @"syncUploadContactUrl",@"getContactListVersionUrl",
                         @"slowSyncUrl",
                         @"syncPortraitUrl",@"clientReportUrl",@"userCloudSummaryUrl",nil];
    NSLog(@"urls: %@",configResponseArray);
...
- (BOOL)syncGetConfig
{
    ...
    if (!error)
	{
		...
		@catch (NSException * e)
		{
			vosLog(@"syncGetConfig Failed. NSException:%@", [e name]);
            memset(&sse, 0, sizeof(sse)); //Added by Kevin on June,20 2013
    ...
    vosLog(@"syncGetConfig Failed.reason=%d", [error code]);
...
- (void)syncAuthen:(SyncTaskType)syncTaskType;
{
	...
	//vosLog(@"account:%@ password:%@", account, password);
                    ...
                    TaskType:(SyncTaskType)syncTaskType
{
	DownloadPortraitRequest *_downloadPortraitRequest = [[[[DownloadPortraitRequest builder]
                                                         ...
                                                         build];
	vosLog(@"%@: syncDownloadPortrait:(%d).", [arrayTaskString objectAtIndex:syncTaskType], [contactSummaryList count]);
                  ...
                  TaskType:(SyncTaskType)syncTaskType
{
	...
	vosLog(@"%@: syncUploadPortrait:(%d).", [arrayTaskString objectAtIndex:syncTaskType], [mutablePortraitList count]);
...
- (void)syncAuthenFinished:(ASIHTTPRequest *)request
{
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncAuthenFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
...
{
	vosLog(@"%@:syncGetContactSummaryFinished.", [arrayTaskString objectAtIndex:request.syncTaskType]);
    NSData *responseData = [request responseData];
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncGetContactSummaryFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
    ...
    switch (request.syncTaskType)
    {
        case TASK_ALL_DOWNLOAD:
            ...
            vosLog(@"syncGetContactSummaryFinished contactListVersion=%d contact_count=%d group_count=%d", [_contactListSummaryResponse contactListVersion], [[_contactListSummaryResponse contactSummaryList] count], [[_contactListSummaryResponse groupSummaryList] count] );
...
{
    vosLog(@"%@:syncDownloadContactFinished.", [arrayTaskString objectAtIndex:request.syncTaskType]);
    NSData *responseData = [request responseData];
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncDownloadContactFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
		[self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
		return;
	}
	@finally{
...
#if DEBUG
    vosLog(@"%@: total download %d contact, %d group.", [arrayTaskString objectAtIndex:request.syncTaskType], [[_syncDownloadContactResponse contactList] count], [[_syncDownloadContactResponse groupList] count]);
#endif
	...
	[self syncStatusRefresh:YES];
	//Bug 所在点
	vosLog(@"%@: 入库结束. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
...
- (void)syncGetVersionFinished:(ASIHTTPRequest *)request // 获取联系人版本号结束
{
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncGetVersionFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
		[self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
		return;
	}
	@finally
   ...
   // int32_t contactListVersion = _getContactListVersionReponse.contactListVersion;
    vosLog(@"%@: syncGetVersionFinished. servre version=%d， local version=%d", [arrayTaskString objectAtIndex:request.syncTaskType], contactListVersion, [self getContactListVersion]);
...
//                                     forDomain:[NSString stringWithFormat:@"SyncClientInfo"]] intValue];
    ...
    {
        case 0:{//本地AB没有变化
			vosLog(@"%@: diffSnapshot==0", [arrayTaskString objectAtIndex:request.syncTaskType]);        
			
            break;
        }
        case -1:{  //本地AB发生错误，所有同步操作都改为慢同步
			vosLog(@"%@: diffSnapshot==-1", [arrayTaskString objectAtIndex:request.syncTaskType]);
            [[MemAddressBook getInstance] rollback];
            ...
            [self syncUPloadDifferContact:[[MemAddressBook getInstance] contactAddList]
                       ...
                       ContactListSummary:[[MemAddressBook getInstance] contactListSummary]
                         GroupListSummary:[[MemAddressBook getInstance] groupListSummary]
                             BusinessCard:[[MemAddressBook getInstance] myCard]
                      BusinessCardVersion:[[MemAddressBook getInstance] myCardVersion]
                                 TaskType:TASK_DIFFER_SYNC];
			break;
        }
        case TASK_MERGE_SYNC:{   // 慢同步
...
- (void)syncDownloadPortraitFinished:(ASIHTTPRequest *)request
{
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncDownloadPortraitFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
	...
	vosLog(@"%@: syncDownloadPortraitFinished. downloadPortrait:%d, businessCardPortrait Version:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [mutablePortraitList count], portraitVersion);
	...
	vosLog(@"%@: 任务成功. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
}
...
- (void)syncPortraitSummaryFinished:(ASIHTTPRequest *)request
{
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncPortraitSummaryFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
		[self procPortraitFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
		return;
	}
	@finally
	...
	vosLog(@"%@: syncPortraitSummaryFinished. 服务端通知客户端 downloadPortrait:%d,deletedPortrait：%d", [arrayTaskString objectAtIndex:request.syncTaskType], [downloadPortraitIdList count], [deletedPortraitIdList count]);
	// 根据服务端的通知删除头像
...
- (void)syncUploadPortraitFinished:(ASIHTTPRequest *)request
{
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncUploadPortraitFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
		[self procPortraitFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
		return;
	}
	@finally
...
#if DEBUG
	vosLog(@"%@: syncUploadPortraitFinished. 服务端通知客户端 updPortrait:%d, businessCardPortraitVersion:%d", [arrayTaskString objectAtIndex:request.syncTaskType], [[_uploadPortraitResponse portraitSummaryList] count], [_uploadPortraitResponse businessCardPortraitVersion]);
#endif
    ...
    switch (request.syncTaskType)
    {
        case TASK_ALL_UPLOAD:// 全量上传
			...
			vosLog(@"%@:syncUploadPortraitFinished commit finished. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
            break;
		case TASK_DIFFER_SYNC:// 双向同步
			memset(&sse, 0, sizeof(sse));
...
- (void)syncUPloadDifferContactFinished:(ASIHTTPRequest *)request
{
	...
	vosLog(@"%@: syncUPloadDifferContactFinished. responseStatusCode:%d", [arrayTaskString objectAtIndex:request.syncTaskType], responseStatusCode);
    ...
    if (responseStatusCode == 204) // 服务端要求执行慢同步
    {
        ...
        vosLog(@"%@ change to %@", [arrayTaskString objectAtIndex:request.syncTaskType], [arrayTaskString objectAtIndex:TASK_MERGE_SYNC]);
	...
	@catch (NSException * e)
	{
		vosLog(@"%@: syncUPloadDifferContactFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
		[self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
		return;
	}
	@finally
    ...
    vosLog(@"Local ContactListVersion: %d", [self getContactListVersion]);
#if DEBUG
	vosLog(@"%@ 服务端通知客户端: contactListVersion:%d, update group:%d, delete group:%d, download group:%d, update contact:%d, delete contact:%d, download contact:%d",
	...
	vosLog(@"%@: 更新客户端版本号结束. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
	...
	switch (request.syncTaskType)
	{
        ...
        case TASK_SYNC_DOANLOAD:// 同步下载
            ...
            vosLog(@"%@: need download %d contact, %d group", [arrayTaskString objectAtIndex:request.syncTaskType], [[_syncUploadContactResponse updatedContactIdList] count], [[_syncUploadContactResponse updatedGroupIdList] count]);
...
- (BOOL)downloadBussinessCard:(SyncTaskType)syncTaskType{
    ...
    if (!error)
    {
        ...
        @catch (NSException * e)
        {
            vosLog(@"%@: batchDownloadContact. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
            [self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
            return result;
        }
        @finally
        ...
        if ([_syncDownloadContactResponse businessCard] != nil){
            vosLog(@"%@: 保存名片.businessCard Version=%d", [arrayTaskString objectAtIndex:request.syncTaskType], [[_syncDownloadContactResponse businessCard] version]);
...
- (void)batchDownloadContact:(NSArray *)contactSummaryList
                 GroupIDList:(NSArray *)groupSummaryList
                    TaskType:(SyncTaskType)syncTaskType
{
	vosLog(@"%@(batch): contactSummaryList(%d), groupSummaryList(%d).", [arrayTaskString objectAtIndex:syncTaskType], [contactSummaryList count], [groupSummaryList count]);
    ...
    {     // 计算本次需要下载的数量
		...
		SyncDownloadContactRequest *_syncDownloadContactRequest = [[[[[SyncDownloadContactRequest builder]
																	...
																	setIsRequestBusinessCard:IsRequestBusinessCard]
																   build];
        ...
        if (!error){
			...
			vosLog(@"%@:batchDownloadContact(groups): %d/%d [%d/%d].", [arrayTaskString objectAtIndex:request.syncTaskType], batchNo, totalBatch, iRecvNum, totalCount);
			NSData *responseData = [request responseData];
		...
		{
			vosLog(@"batchDownloadContact Failed.reason=%d", [error code]);
	...
	{    // 计算本次需要下载的数量
        ...
        if (!error)
		{
			...
			vosLog(@"%@:batchDownloadContact(contacts): %d/%d [%d/%d].", [arrayTaskString objectAtIndex:request.syncTaskType], batchNo, totalBatch, iRecvNum, totalCount);
			NSData *responseData = [request responseData];
			...
			@catch (NSException * e)
			{
				vosLog(@"%@: batchDownloadContact 2. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
				[self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
				return;
			}
			@finally
...
- (void)batchUploadAllContact:(NSArray*)contactList
                 ...
                 BusinessCard:(Contact*)businessCard
                     TaskType:(SyncTaskType)syncTaskType
{
	vosLog(@"%@(batch): addAllContactInfo(%d), addAllGroupInfo(%d).", [arrayTaskString objectAtIndex:syncTaskType], [contactList count], [groupList count]);
	...
	while (remainUploadNum > 0 || (remainUploadNum == 0 && batchNo == 0))
    {
        ...
        vosLog(@"%@(batch): batchNo=%d, groupsUpload=%d, contactsUpload=%d.", [arrayTaskString objectAtIndex:syncTaskType], batchNo, nUpload_g, nUpload_c);
        ...
        if (batchNo >= totalBatch){     // 最后一批
            _uploadContactInfoRequest = [[[[[UploadContactInfoRequest builder]
										   ...
										   addAllGroupInfo:(nUpload_g > 0 ? groupsUpload : nil)]
		...
		if (!error)
		{
			...
			vosLog(@"%@:batchUploadContact(groups:%d, contacts:%d): %d/%d [%d/%d].", [arrayTaskString objectAtIndex:request.syncTaskType], nUpload_g, nUpload_c, batchNo, totalBatch, iSendNum, totalCount);
			...
			switch (request.syncTaskType)  //分全量上传、慢同步
			{
					...
					@try
                {
					...
					@catch (NSException * e)
                {
                    vosLog(@"%@: batchUploadAllContact. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
                    [self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
                    return;
                }
					@finally
                {
					...
					if (totalBatch <= batchNo){  // 在最后一片结束时处理入库
                        ...
                        vosLog(@"%@:batchUploadContact commit finished. ", [arrayTaskString objectAtIndex:syncTaskType]);
						...
						if ([SettingInfo getIsSyncHeadimg]){  //头像同步
							...
							[self syncUploadPortrait:[[MemAddressBook getInstance] contactAddList]
								BusinessCardPortrait:[[MemAddressBook getInstance] myPortrait]
											TaskType:(SyncTaskType)syncTaskType];
						}
						else{
							sse.status = Sync_State_Success;
					...
					@try
                {
					...
					@catch (NSException * e)
                {
                    vosLog(@"%@: batchUploadAllContact 2. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
                    [self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
					...
					@finally
                {
                    ...
                    vosLog(@"%@(batch) 服务端通知客户端: contactListVersion:%d, update group:%d, delete group:%d, download group:%d, update contact:%d, delete contact:%d, download contact:%d",
                           [arrayTaskString objectAtIndex:request.syncTaskType],
                           [_syncUploadContactResponse contactListVersion],
                           [[_syncUploadContactResponse groupMappingInfoList] count],
                           [[_syncUploadContactResponse deletedGroupIdList] count],
					...
					if (totalBatch <= batchNo){
                        ...
                        vosLog(@"%@(batch): 已保存服务端通知的更新，还没有commit，因为后续需要下载. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
                        ...
                        // 根据返回结果确定需要下载的联系人和组
                        // 仍使用原有的不分批次下载方式
                        vosLog(@"%@(batch): need download %d contact, %d group", [arrayTaskString objectAtIndex:request.syncTaskType], [[_syncUploadContactResponse updatedContactIdList] count], [[_syncUploadContactResponse updatedGroupIdList] count]);
                        [self syncDownloadContact:[_syncUploadContactResponse updatedContactIdList]
                                      GroupIDList:[_syncUploadContactResponse updatedGroupIdList]
                                         TaskType:TASK_MERGE_SYNC];
                    }
		...
		{
			vosLog(@"batchUploadContact Failed.reason=%d", [error code]);
			[self procABFailed:request ErrCode:[error code]];
...
- (void)jumpToPortraitSync:(SyncTaskType)syncTaskType
{
    if (![SettingInfo getIsSyncHeadimg])
    {
        ...
        vosLog(@"%@: jumpToPortraitSync Failed. [SettingInfo getIsSyncHeadimg] is not true.", [arrayTaskString objectAtIndex:syncTaskType]);
        return;
    }
...
- (void)syncUploadAllContactFinished:(ASIHTTPRequest *)request
{
    ...
    vosLog(@"%@: syncUploadAllContactFinished. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
    ...
    switch (request.syncTaskType)
	{
			...
			@try
        {
			...
			@catch (NSException * e)
        {
            vosLog(@"%@: syncUploadAllContactFinished. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
            [self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
            return;
        }
			@finally
        {
			...
			vosLog(@"%@: 结束. ", [arrayTaskString objectAtIndex:request.syncTaskType]);
            /////////////////////add code here///////////////////
		...
		case TASK_MERGE_SYNC:{ // 慢同步
			...
			@catch (NSException * e)
			{
				vosLog(@"%@: syncUploadAllContactFinished 2. NSException:%@", [arrayTaskString objectAtIndex:request.syncTaskType], [e name]);
				[self procABFailed:request ErrCode:SYNC_STATE_RESPONSE_BAD_GATEWAY];
