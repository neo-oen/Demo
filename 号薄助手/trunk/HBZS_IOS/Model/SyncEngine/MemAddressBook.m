...
+ (void)initialize
{
    ...
    if(!initialized)
    {
		...
		vosLog(@"MemAddressBook instance=%@", theMAB);
    }
}
        ...
        for (Person2Group *pg in contactBelongToGroupArray){
            if (pg.group && pg.person){
                ...
                @catch (NSException *exception) {
                    vosLog(@"ABGroupAddMember Failed. NSException:%@", [e name]);
                    // break;
                }
                @finally {
...
- (int)getAddressDbChange{
    ...
    vosLog(@"MemAB:diffSnapshot: ag=%d ac=%d ug=%d uc=%d dg=%d dc=%d ssg=%d ssc=%d", [addGroupArray count], [addContactArray count], [updGroupArray count], [updContactArray count], [delGroupArray count], [delContactArray count], [groupSummaryArray count], [contactSummaryArray count]);
   // [contactBelongToGroupDict release];
...
- (int32_t)addGroup:(Group*)g {
    ...
    if(group == nil){
        vosLog(@"MemAB::addGroup, create group == nil");
...
- (int32_t)addContact:(Contact*)c
{
    ...
    if(person == nil){
        vosLog(@"MemAB::addContact,person == nil");
        return DATA_ERR_FAIL_ADD_RECORD;
    }
	// Add person to AB
	CFErrorRef error = nil;
...
- (int32_t)updGroup:(Group*)g
{
    ...
    vosLog(@"MemAB:updGroup sid=%d", gID);
    ...
    if (mrIndex < 0){
        vosLog(@"MemAB:updGroup memRecord not found.gID=%d", gID);
...
- (int32_t)updContact:(Contact*)c
{
    ...
    if (mrIndex < 0) {
        vosLog(@"MemAB:updContact memRecord not found.cID=%d", cID);
        return DATA_ERR_FAIL_REPLACE_RECORD;
    }
...
- (int32_t)delGroup:(int32_t)gID
{
    ...
    if (mrIndex < 0) {
        vosLog(@"MemAB:delGroup memRecord not found.gID=%d", gID);
        return DATA_ERR_FAIL_REPLACE_RECORD;
    }
...
- (int32_t)delContact:(int32_t)cID
{
    ...
    if (mrIndex < 0){
        vosLog(@"MemAB:delContact memRecord not found.cID=%d", cID);
        return DATA_ERR_FAIL_REPLACE_RECORD;
    }
...
- (Contact*)myCard{
	...
	vosLog(@"load BusinessCard from file: %@", documentsDirectory);
...
- (int32_t)updMyCard:(Contact*)varCard;
{
	if (varCard != nil){
        ...
        vosLog(@"save to BusinessCard file: %@", documentsDirectory);
...
- (PortraitData*)myPortrait
{
    ...
    vosLog(@"load BusinessCardPortrait from file: %@", documentsDirectory);
...
- (int32_t)updMyPortrait:(PortraitData*)varPortrait
{
	if (varPortrait != nil)
	{
        ...
        vosLog(@"save to BusinessCardPortrait file: %@", documentsDirectory);
...
- (int32_t)_updRecord:(int32_t)serverId WithVersion:(int32_t)version WithTempID:(int32_t)tempID IsGroup:(BOOL)isGroup
{
    ...
    if (index >= count){
        // 下标越界
        vosLog(@"updContact error index=%d", tempID);
...
- (int32_t)startPortraitSyncTask:(SyncTaskType)task{
    vosLog(@"MemAB::startPortraitSyncTask task=%d", task);
    int32_t ret = 0;
...
{
    vosLog(@"MemAB::commitPortraitTask");
...
{
    vosLog(@"MemAB::rollbackPortraitTask");
    ABAddressBookRevert(abRef);
...
{
    vosLog(@"MemAB::unchangedPortraitTask");
    [self clearTempResultList];
    // 恢复备份的快照列表。
    if (arrayData)
    {
...
- (int32_t)getAllPortraits
{
    ...
    for (MemRecord* mRec in arrayData){
        ...
        if (person == nil){
            vosLog(@"MemAB:getAllPortraits can not load record:%d, %d, %d, %d, %@", mRec.realABID, mRec.serverId, mRec.version, mRec.portraitVersion, mRec.portraitMD5);
            continue;
        }
...
- (int32_t)updPortrait:(DownloadPortraitData*)portrait
{
    ...
    {
        vosLog(@"MemAB:updPortrait can not find portrait. %d", sid);
        return SYNC_ERR_WRONG_PARAM;
    }
    ...
    if (mRec == nil)
    {
        vosLog(@"MemAB:updPortrait can not find memAB record by %d", sid);
        return SYNC_ERR_WRONG_PARAM;
    }
    ...
    if (person == nil)
    {
        vosLog(@"MemAB:updPortrait can not find AB person record by %d, %d.", sid, mRec.realABID);
        return SYNC_ERR_WRONG_PARAM;
    }
...
- (int32_t)delPortrait:(int32_t)serverId
{
    ...
    if (mRec == nil){
        vosLog(@"MemAB:delPortrait can not find memAB record by %d", serverId);
        return SYNC_ERR_WRONG_PARAM;
    }
    ...
    if (person==nil)
    {
        vosLog(@"MemAB:delPortrait can't load record:%d, %d, %d", mRec.realABID, mRec.serverId, mRec.version);
        return SYNC_ERR_WRONG_PARAM;
    }
...
- (int)compareContact:(ABRecordRef)contact ToMemRecord:(MemRecord*)mRec
{
    ...
    if (fullContentMD5 != nil){
        ...
        vosLog(@"MemAB::compareContact recordID=%@ oldMD5=%@ newMD5=%@ c=%@", key, fullContentMD5, newFullContentMD5, [c data]);
        [key release];
        return 1;
    }
    ...
    {
		vosLog(@"修改时间：ABRecordDate=%@, lastCheckModifyDate=%@", date, mRec.lastCheckModifyDate);
        CFRelease(date);
         [key release];
    ...
    {
		vosLog(@"创建时间：ABRecordDate=%@, lastCheckCreateDate=%@", date, mRec.lastCheckCreateDate);
        CFRelease(date);
         [key release];
        return 1;
    }
...
////////
    ...
    vosLog(@"buildFullSnapshot......3...");
    [self saveFullSnapshot];
    vosLog(@"buildFullSnapshot......4...");
}
