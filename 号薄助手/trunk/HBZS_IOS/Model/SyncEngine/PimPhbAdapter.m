...
NSString* setPhoneBuilder(ABRecordRef record, Contact_Builder *cb){
    NSString *value;
    ...
    for(int i = 0 ;i < nCount; i++){
       ...
       // NSLog(@"lable: %@, number: %@", lable, value);
        if([lable isEqualToString:(NSString *)kABPersonPhoneMobileLabel]){//_$!<Mobile>!$_
            [cb setMobilePhone:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABPersonPhoneIPhoneLabel]){//iPhone
            [cb setWorkMobilePhone:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABPersonPhoneMainLabel]){//_$!<Main>!$_
            [cb setHomeTelephone:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABPersonPhoneHomeFAXLabel]){//_$!<HomeFAX>!$_
            [cb setHomeFax:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABPersonPhoneWorkFAXLabel]){//_$!<WorkFAX>!$_
            [cb setFax:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABWorkLabel]){//_$!<Work>!$_
            [cb setWorkTelephone:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABHomeLabel]){//_$!<Home>!$_
            [cb setTelephone:[[[Phone builder] setPhoneValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kSABVPNLabel]){
...
NSString* setEmailBuilder(ABRecordRef record, Contact_Builder *cb){
   int _index = 1;
    // mails
    
    ABMultiValueRef mails = (ABMultiValueRef) ABRecordCopyValue(record, kABPersonEmailProperty);
    
   int nCount = ABMultiValueGetCount(mails);
    ...
    for(int i = 0 ;i < nCount; i++){
        ...
        if([lable isEqualToString:(NSString *)kABHomeLabel]){
            [cb setEmail:[[[Email builder] setEmailValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABWorkLabel]){
            [cb setWorkEmail:[[[Email builder] setEmailValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kABOtherLabel]){
		...
		else if([lable isEqualToString:(NSString *)kSABQQLabel]){
            [cb setQq:[[[InstantMessage builder] setImValue:value] build]];
		}
		else if([lable isEqualToString:(NSString *)kSABMSNLabel]){
...
NSString* setDateBuilder(ABRecordRef record){
	...
	int nCount = ABMultiValueGetCount(dates);
...
//        }
        ...
        if([service isEqualToString:@"MSN"]){
            [cb setMsn:[[[InstantMessage builder] setImValue:uvalue] build]];
        }
        else if([service isEqualToString:@"QQ"]){
            [cb setQq:[[[InstantMessage builder] setImValue:uvalue] build]];
        }
        else{
            NSString *key = [NSString stringWithFormat:@"其它即时通信方式%d",_index];
...
UInt32 PimPhb_ABRecord2Contact(ABRecordRef record, Contact_Builder* cb, NSMutableDictionary* contat2groupDict)
{
    ...
    NSString *temp1 = setEmailBuilder(record, cb);
    
    if (temp1 != nil) {
        comment = [comment stringByAppendingString:temp1];         //Email Builder
    }
    
    NSString *temp2 = setPhoneBuilder(record, cb);
    if (temp2 != nil) {
        comment = [comment stringByAppendingString:temp2];         //Phone Builder
    }
...
void setPersonPhoneValue(Contact* pContact, ABRecordRef person){
   ...
   // NSLog(@"FAX: %@", nsStr);
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiPhone, nsStr, kABPersonPhoneWorkFAXLabel, NULL);  //fax
    }
  ...
  //   NSLog(@"homeFax: %@", nsStr);
    ...
    ////服务端以workFax下发其他邮箱1、ePhone下发其他邮箱2
    nsStr = [[pContact workFax] phoneValue];
  //  NSLog(@"workFax: %@", nsStr);
    if(nil != nsStr && 0 < [nsStr length]){
       // NSLog(@"##### workFax->其他邮箱1: %@", nsStr);
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        [userDefaults setObject:nsStr forKey:@"otherEmail1"];
        [userDefaults synchronize];
        //ABMultiValueAddValueAndLabel(multiPhone, nsStr, (CFStringRef)kSABFaxLabel, NULL); //workFax
    }
    ...
    if(nil != nsStr && 0 < [nsStr length]){
       // NSLog(@"#### ePhone->其他邮箱2: %@", nsStr);
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        [userDefaults setObject:nsStr forKey:@"otherEmail2"];
        [userDefaults synchronize];
    }
    ...
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiPhone, nsStr, (CFStringRef)kSABVPNLabel, NULL);  //vpn
    }
...
void setPersonEmailValue(Contact* pContact, ABRecordRef person){
    ...
    ////服务端以workFax、ePhone字段下发其他邮箱1、其他邮箱2
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    nsStr = [userDefaults objectForKey:@"otherEmail1"];
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiEmail, (CFStringRef)nsStr, (CFStringRef)@"其它邮箱1", NULL);  //comEmail
        [userDefaults removeObjectForKey:@"otherEmail1"];
        [userDefaults synchronize];
    }
    ...
    ///////  QQ MSN Blood     ////
    nsStr = [[pContact qq] imValue];
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiEmail, (CFStringRef)nsStr, (CFStringRef)kSABQQLabel, NULL);  //QQ
    }
...
void setPersonQqMsnGenderConstellationBloodValue(Contact* pContact, ABRecordRef person){
    ...
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiqq, (CFMutableStringRef)nsStr, (CFStringRef)kSABQQLabel, NULL);  //QQ
    }
    
    nsStr = [[pContact msn] imValue];
    
    if(nil != nsStr && 0 < [nsStr length]){
        ABMultiValueAddValueAndLabel(multiqq, nsStr, (CFStringRef)kSABMSNLabel, NULL);     //MSN
    }
    ...
    if (ABMultiValueGetCount(multiqq) > 0){
        ABRecordSetValue(person, kABPersonEmailProperty, multiqq, &error);        //kABPersonEmailProperty
	}
...
UInt32 PimPhb_Contact2ABRecord(Contact* pContact, ABRecordRef person)
{
    ...
    if (type == kABGroupType){
		vosLog(@"PimPhb_Contact2ABRecord  失败：type == kABGroupType");
		return SML_ERR_WRONG_PARAM;
	}
...
UInt32 PimPhb_AddGroup(ABAddressBookRef abRef, Group* pGroup) {
 //   vosLog(@"PimPhbGroup_Add");
    ...
    if(group == nil){
        vosLog(@"PimPhb_AddGroup, group == nil");
...
UInt32 PimPhb_AddContact(ABAddressBookRef abRef, Contact* pContact) {
  ...
  //  vosLog(@"phb add one contact start");
    ABRecordRef person = ABPersonCreate();
    
    if(person == nil)
    {
        vosLog(@"PimPhb_Add,person == nil");
        if (person) {
            CFRelease(person);
        }
