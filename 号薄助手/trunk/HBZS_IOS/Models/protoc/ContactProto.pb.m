...
@property (retain) InstantMessage* yixin;
@property (retain) InstantMessage* weixin;
...
- (void) setHasCardName:(BOOL) value {
  hasCardName_ = !!value;
}
@synthesize cardName;
- (BOOL) hasTimestamp {
...
- (id) init {
  if ((self = [super init])) {
    ...
    self.lastSource = 0;
    self.lunarBirthday = @"";
    self.yixin = [InstantMessage defaultInstance];
    self.weixin = [InstantMessage defaultInstance];
    self.cardSid = 0;
    self.cardName = @"";
    self.timestamp = 0L;
  }
...
- (void) writeToCodedOutputStream:(PBCodedOutputStream*) output {
  ...
  if (self.hasYixin) {
    [output writeMessage:35 value:self.yixin];
  }
  if (self.hasWeixin) {
    [output writeMessage:36 value:self.weixin];
  }
  if (self.hasCardSid) {
    [output writeInt32:37 value:self.cardSid];
  }
  if (self.hasCardName) {
    [output writeString:38 value:self.cardName];
  }
  if (self.hasTimestamp) {
    [output writeInt64:39 value:self.timestamp];
  }
  [self.unknownFields writeToCodedOutputStream:output];
}
- (int32_t) serializedSize {
  ...
  if (self.hasYixin) {
    size += computeMessageSize(35, self.yixin);
  }
  if (self.hasWeixin) {
    size += computeMessageSize(36, self.weixin);
  }
  if (self.hasCardSid) {
    size += computeInt32Size(37, self.cardSid);
  }
  if (self.hasCardName) {
    size += computeStringSize(38, self.cardName);
  }
  if (self.hasTimestamp) {
    size += computeInt64Size(39, self.timestamp);
  }
  size += self.unknownFields.serializedSize;
  memoizedSerializedSize = size;
  return size;
...
- (Contact_Builder*) mergeFrom:(Contact*) other {
  ...
  if (other.hasYixin) {
    [self mergeYixin:other.yixin];
  }
  if (other.hasWeixin) {
    [self mergeWeixin:other.weixin];
  }
  if (other.hasCardSid) {
    [self setCardSid:other.cardSid];
  }
  if (other.hasCardName) {
    [self setCardName:other.cardName];
  }
  ...
  while (YES) {
    ...
    switch (tag) {
      ...
      case 282: {
        InstantMessage_Builder* subBuilder = [InstantMessage builder];
        if (self.hasYixin) {
          [subBuilder mergeFrom:self.yixin];
        }
        [input readMessage:subBuilder extensionRegistry:extensionRegistry];
        [self setYixin:[subBuilder buildPartial]];
        break;
      }
      case 290: {
        InstantMessage_Builder* subBuilder = [InstantMessage builder];
        if (self.hasWeixin) {
          [subBuilder mergeFrom:self.weixin];
        }
        [input readMessage:subBuilder extensionRegistry:extensionRegistry];
        [self setWeixin:[subBuilder buildPartial]];
        break;
      }
      case 296: {
      ...
      case 306: {
        [self setCardName:[input readString]];
        break;
      }
      case 312: {
...
- (Contact_Builder*) setYixinBuilder:(InstantMessage_Builder*) builderForValue {
  return [self setYixin:[builderForValue build]];
}
- (Contact_Builder*) mergeYixin:(InstantMessage*) value {
  if (result.hasYixin &&
      result.yixin != [InstantMessage defaultInstance]) {
    result.yixin =
      [[[InstantMessage builderWithPrototype:result.yixin] mergeFrom:value] buildPartial];
  } else {
    result.yixin = value;
  }
  result.hasYixin = YES;
...
- (BOOL) hasWeixin {
  return result.hasWeixin;
}
- (InstantMessage*) weixin {
  return result.weixin;
}
- (Contact_Builder*) setWeixin:(InstantMessage*) value {
  result.hasWeixin = YES;
  result.weixin = value;
  return self;
}
- (Contact_Builder*) setWeixinBuilder:(InstantMessage_Builder*) builderForValue {
  return [self setWeixin:[builderForValue build]];
}
- (Contact_Builder*) mergeWeixin:(InstantMessage*) value {
  if (result.hasWeixin &&
      result.weixin != [InstantMessage defaultInstance]) {
    result.weixin =
      [[[InstantMessage builderWithPrototype:result.weixin] mergeFrom:value] buildPartial];
  } else {
    result.weixin = value;
  }
  result.hasWeixin = YES;
