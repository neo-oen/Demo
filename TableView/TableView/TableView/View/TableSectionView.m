...
{
    if(!_tableView) {
        _tableView = [[WYQBaseTableView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height) style:_style];
    //是否等宽高
...
+(TableSectionView *)TableSectionWithStyle:(UITableViewStyle)style andModel:(NSArray *)models{
    ...
    TableSectionView.models = models;
    TableSectionView.style = style;
    return TableSectionView;
}
